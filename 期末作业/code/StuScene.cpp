...
StuScene::StuScene(School *sch,QWidget *parent) : QWidget(parent)
{
    setFixedSize(560,360);
    setWindowTitle("学生信息");
    setWindowIcon(QPixmap(":/stu.png"));

    //下拉框 设置查询方式
    QComboBox *select = new QComboBox(this);
    QStringList list = {"按学号查询","按姓名查询"};
    ...
    view->setGeometry(20,70,446,220);
    ...
    connect(show_btn,&QPushButton::clicked,[=]()
    {
        ...
        if(sch->db.open())
        {
            ...
            model->setQuery("select * from student", sch->db);
            /*  ---设置表头---   */
    ...
    connect(sel_btn,&QPushButton::clicked,[=]()
    {
        ...
        {
            QSqlQuery sql;
            if(select->currentIndex() == 0)//按学号方式进行查询
            {
                if(!edit->text().isEmpty())//输入框有键入
                {
                    bool is_exec = sql.exec("select *from student where id = '"+edit->text()+"'");
                    if(!is_exec)
                    {
            ...
            else if(select->currentIndex() == 1)//按姓名方式进行查询
            {
                if(!edit->text().isEmpty())//输入框有键入
                {
                    bool is_exec = sql.exec("select *from student where name = '"+edit->text()+"'");
                    if(!is_exec)
                    {
                        qDebug()<<"查询失败"<<sql.lastError();
                    }
...
void StuScene::paintEvent(QPaintEvent *)
{
